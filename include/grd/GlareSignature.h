...
public:
   ...
   */
  const Eigen::MatrixXd& getSignatureGlobal() const { return signatureGlobal_; }

  /** Number of points used in the signature.
   */
  int getPointNum() const { return signaturePoints_.size(); }

 /** Returns a reference to the point i-th.
   */
  const Eigen::Vector2d& getPoint(int i) const { return signaturePoints_[i].point; }

 /** Returns a reference to the signature of point i. 
   */
  const Eigen::MatrixXd& getSignaturePoint(int i) const { return signaturePoints_[i].signature; }
